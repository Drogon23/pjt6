...
function loadCategories(){
	...
	oReq.addEventListener("load", function() {
		...
		data.items.forEach(function(cate){
			...
			templete = templete.replace("{id}", cate.id).replace("{name}", cate.name);
...
let oReq = new XMLHttpRequest();
	...
	oReq.addEventListener("load", function() {
		...
		data.items.forEach(function(promotion){
			...
			templete = templete.replace("{productId}", promotion.productId).replace("{productImageId}", promotion.productImageId);
			parent.innerHTML += templete;
		});
