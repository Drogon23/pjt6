    ...
    <div id="container">
        ...
        <div class="header fade">
            <header class="header_tit">
                ...
                <a class="btn_my"> <span title="예약확인"><c:if test = "${rsvEmail == ''}">예약확인</c:if>${rsvEmail}</span> </a>
            </header>
        </div>
        <div class="ct">
            <div class="ct_wrap">
                        ...
                        <ul class="visual_img">
                            <li class="item" style="width: 414px;"> <img alt="" class="img_thumb" src="/productImages/${product.id}/ma"> <span class="img_bg"></span>
                                <div class="preview_txt">
                                    <h2 class="preview_txt_tit">${product.description}</h2> 
                                    <em class="preview_txt_dsc">
                                    	<c:forEach var="price" items="${productPrices}" varStatus="status">
			                            	<c:if test = "${status.first}">
			                            		₩${String.format("%,d",price.price)} ~ 
			                            	</c:if>			                            		
                ...
                <div class="section_store_details">
                    <div class="store_details">
                        ...
                        <p class="dsc">
                        <c:forEach var="price" items="${productPrices}">
                            ${price.priceTypeName} : ${String.format("%,d",price.price)} <c:if test="${0 ne price.discountRate}">(${price.discountRate}% 할인가)</c:if><br>
                        </c:forEach> 
                        </p>
                    </div>
                </div>
                        	...
                        	<div class="qty">
                                ...
                                <!-- [D] 수량이 최소 값이 일때 ico_minus3, count_control_input에 disabled 각각 추가, 수량이 최대 값일 때는 ico_plus3에 disabled 추가 -->
                                	<div class="clearfix">
                                    	...
                                    	<input type="tel" class="count_control_input disabled" value="0" readonly title="수량" data-id="${price.id}">
                           		...
                           		<div class="qty_info_icon"> 
	                            	...
	                            	<strong class="product_price"> <span class="price">${String.format("%,d",price.price)}</span> <span class="price_type">원</span> </strong> 
	                            	<em class="product_dsc">${String.format("%,d",price.price)}원 <c:if test="${0 ne price.discountRate}">(${price.discountRate}% 할인가)</c:if></em> 
                            	</div>
                       		</div>
                        </c:forEach> 
                    ...
                    <div class="booking_form_wrap">
                        <div class="form_wrap">
                            ...
                            <form class="form_horizontal">
                          		...
                          		<input type="hidden" name="displayInfoId" value ="${displayInfoId}" />
                                <div class="inline_form"> <label class="label" for="name"> <span class="spr_book ico_nessasary">필수</span> <span>예매자</span> </label>
                                    ...
                                    <div class="warning_msg">형식이 틀렸어요</div>
                                    </div>
                                </div>
                                <div class="inline_form"> <label class="label" for="tel"> <span class="spr_book ico_nessasary">필수</span> <span>연락처</span> </label>
                                    <div class="inline_control tel_wrap">
                                        ...
                                        <div class="warning_msg">000-0000-0000 형식으로 입력해주세요</div>
                                    </div>
                                </div>
                                ...
                                <div class="inline_form last"> <label class="label" for="message">예매내용</label>
                                    <div class="inline_control">
                                        <p class="inline_txt selected">${currentDate}, 총 <span id="totalCount">0</span>매</p>
                                        <input type="hidden" name=reservationYearMonthDay value ="${currentDate}" />
                                    </div>
                                </div>
